...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 15, 2022 at 03:11 AM
...
--
-- Stand-in structure for view `customer_vw`
-- (See below for the actual view)
--
CREATE TABLE `customer_vw` (
...
--
-- Structure for view `customer_vw`
--
DROP TABLE IF EXISTS `customer_vw`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customer_vw`  AS SELECT `o`.`addressLine1` AS `addressLine1`, `o`.`phone` AS `phone`, `o`.`city` AS `kota`, `e`.`firstName` AS `firstName`, `e`.`email` AS `email`, `e`.`jobTitle` AS `jobTitle`, `c`.`customerName` AS `customerName`, `c`.`phone` AS `telp`, `c`.`city` AS `city`, `p`.`paymentDate` AS `paymentDate`, `p`.`amount` AS `amount` FROM (((`employees` `e` join `offices` `o` on(`e`.`officeCode` = `o`.`officeCode`)) join `customers` `c` on(`e`.`employeeNumber` = `c`.`salesRepEmployeeNumber`)) join `payments` `p` on(`c`.`customerNumber` = `p`.`customerNumber`))  ;
